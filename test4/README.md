...
**第一步：以用户名：studenthuang，密码：123登录数据库huang_pdborcl**
...
**第6步：创建ORDERS表, ORDER_ID_TEMP表并且为ORDERS增加了外键和主键约束**
...
4. 为ORDERS表添加约束
   ...
   **第7步：创建ORDER_DETAILS表**
   ...
   3. 创建本地分区索引ORDER_DETAIL_ORDER_ID
...
2. 递归查询某个员工及其所有下属，子下属员工。（列如1号的员工）
...
3. 查询订单表，并且包括订单的订单应收货款: Trade_Receivable= sum(订单详单表.ProductNum*订单详单表.ProductPrice)- Discount。
   ...
   ```sql
   select  o.order_id "订单编号",o.customer_name "客户姓名",o.customer_tel "客户电话",o.order_date "订单日期",o.employee_id "员工编号",o.discount "折扣", sum(od.PRODUCT_NUM*od.PRODUCT_PRICE-o.Discount) Trade_Receivable 
   from  orders o,order_details od  
   where  o.order_id=od.order_id
   group  by  o.order_id,o.customer_name,o.customer_tel,o.order_date,o.employee_id,o.discount 
...
4. 查询订单详表，要求显示订单的客户名称和客户电话，产品类型用汉字描述。
   ...
   ```sql
    select o.CUSTOMER_NAME ,o.CUSTOMER_TEL ,od.PRODUCT_NAME "产品类型"  
...
5. 查询出所有空订单，即没有订单详单的订单。
   ...
   SELECT  *
   FROM ORDERS
   WHERE ORDER_ID NOT   IN{SELECT B. ORDER_ID
    FROM ORDERS  B, ORDER_DETAILS C 
...
6. 查询部门表，同时显示部门的负责人姓名。
   ...
   ```sql
   SELECT A.DEPARTMENT_NAME,B. NAME
...
```sql
SELECT A.DEPARTMENT_NAME , SUM(C. TRADE_RECEIVABLE)
FROM DEPARTMENTS A，EMPLOYEES B,ORDERS C
WHERE A.DEPARTMENT_ID=B. EMPLOYEE_ID AND B. EMPLOYEE_ID=C.EMPLOYEE_ID
...
通过本次实验我了解到了Oracle表和视图的概念，学习了SQL语句Create Table创建表，Select语句插入，修改，删除以及查询数据，使用SQL语句创建视图，和部分存储过程和触发器的使用，掌握了常用数据库模式对象的管理方法，收获了很多。